...
export class APIClientBase {
  ...
  protected async request<T>(path: string, method = 'GET', payload: any = null, auth = false, reqHeaders = new Headers()) {
    ...
    if (this.config.success !== undefined && !this.config.success(response)) {
      ...
      let errorData = null
      try {
        errorData = (await response.json()) as { title: string; instance: string; detail: string }
      } catch (_err) {
